...
$taskName = "Teams-get-team-channels"
$taskGetTeamsChannelsGuid = ""
try {
    $uri = ($PortalBaseUrl +"api/v1/automationtasks?search=$taskName&container=1")
    $response = (Invoke-RestMethod -Method Get -Uri $uri -Headers $headers -ContentType "application/json" -Verbose:$false) | Where-Object -filter {$_.name -eq $taskName}
...
'@;
        ...
        $uri = ($PortalBaseUrl +"api/v1/automationtasks/powershell")
        $response = Invoke-RestMethod -Method Post -Uri $uri -Headers $headers -ContentType "application/json" -Verbose:$false -Body $body
        $taskGetTeamsChannelsGuid = $response.automationTaskGuid

        Write-ColorOutput Green "Powershell task '$taskName' created: $taskGetTeamsChannelsGuid"   
    } else {
        #Get TaskGUID
        $taskGetTeamsChannelsGuid = $response.automationTaskGuid
        Write-ColorOutput Yellow "Powershell task '$taskName' already exists: $taskGetTeamsChannelsGuid"
    }
} catch {
    Write-ColorOutput Red "Powershell task '$taskName'"
    $_
...
$dataSourceName = "Teams-get-team-channels"
$dataSourceGetTeamsChannelsGuid = ""
try {
    $uri = ($PortalBaseUrl +"api/v1/datasource/named/$dataSourceName")
    $response = Invoke-RestMethod -Method Get -Uri $uri -Headers $headers -ContentType "application/json" -Verbose:$false
    ...
    if([string]::IsNullOrEmpty($response.dataSourceGUID)) {
        ...
        $body = @{
            name = "$dataSourceName";
            type = "3";
            model = @(@{key = "DisplayName"; type = 0}, @{key = "Description"; type = 0}, @{key = "Id"; type = 0});
            automationTaskGUID = "$taskGetTeamsChannelsGuid";
        ...
        $dataSourceGetTeamsChannelsGuid = $response.dataSourceGUID
        Write-ColorOutput Green "Task data source '$dataSourceName' created: $dataSourceGetTeamsChannelsGuid"
    } else {
        #Get DatasourceGUID
        $dataSourceGetTeamsChannelsGuid = $response.dataSourceGUID
        Write-ColorOutput Yellow "Task data source '$dataSourceName' already exists: $dataSourceGetTeamsChannelsGuid"
    }
} catch {
    Write-ColorOutput Red "Task data source '$dataSourceName'"
    $_
...
$formName = "Teams - Remove Team Channel"
$formGuid = ""
try
{
    try {
    ...
    "fields": [
      {
        ...
        "templateOptions": {
          ...
          "dataSourceConfig": {
            "dataSourceGuid": "$dataSourceGetTeamsChannelsGuid",
            "input": {
              "propertyInputs": [
                {
...
$delegatedFormName = "Teams - Remove Team Channel"
$delegatedFormGuid = ""
...
$taskActionName = "Teams-remove-team-channel"
$taskActionGuid = ""
try {
...
'@;
            ...
            variables = @(@{name = "displayName"; value = "{{form.Channels.DisplayName}}"; typeConstraint = "string"; secret = "False"},
